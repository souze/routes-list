...
initialFrontendModel url key shared page =
    ...
    , currentPage = LoginPage emptyLoginPageData
...
loginPageMessage msg loginPageData model =
    case msg of
        LoginPageFieldChange fieldType newValue ->
            ...
            in
            { model | currentPage = LoginPage newLpd }
                |> withNoCommand
...
update msg model =
    case msg of
        FrontendMsgFieldUpdate field newValue ->
            (case ( model.currentPage, field ) of
                ( AdminPage (AdminAddUser username password), FieldAddUserUsername ) ->
                    { model | currentPage = AdminPage (AdminAddUser newValue password) }

                ( AdminPage (AdminAddUser username password), FieldAddUserPassword ) ->
                    { model | currentPage = AdminPage (AdminAddUser username newValue) }

                ( AdminPage (AdminRemoveUser username), _ ) ->
                    { model | currentPage = AdminPage (AdminRemoveUser newValue) }

                ( AdminPage (AdminChangePassword username password), FieldChangePasswordUsername ) ->
                    { model | currentPage = AdminPage (AdminChangePassword newValue password) }

                ( AdminPage (AdminChangePassword username password), FieldChangePasswordPassword ) ->
                    { model | currentPage = AdminPage (AdminChangePassword username newValue) }

                ( ChangePasswordPage attr, FieldUserChangePasswordOldPass ) ->
                    { model | currentPage = ChangePasswordPage { attr | oldPassword = newValue } }

                ( ChangePasswordPage attr, FieldUserChangePasswordNewPass ) ->
                    { model | currentPage = ChangePasswordPage { attr | newPassword = newValue } }

                ( ChangePasswordPage attr, FieldUserChangePasswordNewPass2 ) ->
                    { model | currentPage = ChangePasswordPage { attr | newPassword2 = newValue } }
            ...
            if newPassword == newPassword2 then
                ...
                case model.currentPage of
                    ChangePasswordPage attr ->
                        { model | currentPage = ChangePasswordPage { attr | status = "New passwords don't match" } }
                            |> withNoCommand
        ...
        FrontendMsgAddUser username password ->
            ( { model | currentPage = AdminPage AdminHomePage }
        ...
        FrontendMsgAdminRequestModel ->
            ( { model | currentPage = SpinnerPage "Waiting for model" }
        ...
        FrontendMsgConfirmButtonPressed ->
            case model.currentPage of
                ConfirmPage { text, code, event } ->
                    if text == code then
                        ( { model | currentPage = SpinnerPage "Waiting for backend response" }
        ...
        MoreOptionsButtonPressed ->
            { model | currentPage = MoreOptionsPage }
                |> withNoCommand
        ...
        JsonInputTextChanged newValue ->
            ( case model.currentPage of
                InputJsonPage _ err ->
                    { model | currentPage = InputJsonPage newValue err }
        ...
        ViewAsJsonButtonPressed ->
            ( { model | currentPage = ViewJsonPage }
            , Cmd.none
            )

        InputJsonButtonPressed ->
            ( { model | currentPage = InputJsonPage "" Nothing }
            , Cmd.none
            )

        ViewAllButtonPressed ->
            ( { model | currentPage = RoutePage Types.ViewAll }
            , Cmd.none
            )

        WishlistButtonPressed ->
            ( { model | currentPage = RoutePage Types.ViewWishlist }
            , Cmd.none
            )

        LogButtonPressed ->
            ( { model | currentPage = RoutePage Types.ViewLog }
            , Cmd.none
            )
        ...
        SetCurrentDate date ->
            ( { model
                | currentDate = date
                , currentPage = model.currentPage |> updateCurrentDateForPage date
        ...
        Shared _ ->
            -- TODO, FrontendMsg and FrontendMsg should be different types
            ( model, Cmd.none )
...
trySubmitInputJson text model =
    ...
    case routesOrError of
        ...
        Err err ->
            ( { model | currentPage = InputJsonPage text (Just (Json.Decode.errorToString err)) }
            , Cmd.none
            )
...
createNewRoute model =
    case model.currentPage of
        NewRoutePage { routeData } ->
            -- No validation
            ( { model | currentPage = RoutePage ViewAll }
            , Lamdera.sendToBackend <|
...
updateFromBackend msg model =
    case msg of
        ...
        ToFrontendUserNewPasswordAccepted ->
            case model.currentPage of
                ChangePasswordPage attr ->
                    { model | currentPage = ChangePasswordPage { oldPassword = "", newPassword = "", newPassword2 = "", status = "Success!" } }
                        |> withNoCommand
        ...
        ToFrontendUserNewPasswordRejected ->
            case model.currentPage of
                ChangePasswordPage attr ->
                    { model | currentPage = ChangePasswordPage { attr | status = "That didn't work for some reason!" } }
                        |> withNoCommand
        ...
        ToFrontendAdminWholeModel backupModel ->
            ( { model | currentPage = AdminPage (AdminShowJson (backupModelToJsonStr backupModel)) }
            , Cmd.none
            )
        ...
        ToFrontendWrongUserNamePassword ->
            ( { model | currentPage = LoginPage emptyLoginPageData }
            , Cmd.none
            )
